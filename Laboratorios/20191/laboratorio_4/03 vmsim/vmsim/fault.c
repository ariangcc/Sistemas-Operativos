...
static void fault_second(pte_t *pte, ref_kind_t type){
  ...
  //si no hay frames libres se necesita reemplazar
    ...
    while(TRUE){
      ...
      //se actualiza la posicion en la tabla de memoria fisica 
...
se suma R en cada referencia a todos los contadores de los elementos que se encuentran
...
se separa la memoria en cuatro clases y se toma aleatoriamente uno(en este caso se toma el primer 
elemento de esa clase) y se toma luego el de menor clase 
...
void fault_nru(pte_t *pte, ref_kind_t type){
  ...
  //si no hay frames libres se necesita reemplazar
    ...
    for(int i=0;i<opts.phys_pages;i++) {
      ...
      if(loc1 == -1 &&  physmem[i]->modified  && !physmem[i]->reference) loc1 = i; 
      if(loc2 == -1 && !physmem[i]->modified  &&  physmem[i]->reference) loc2 = i; 
      if(loc3 == -1 &&  physmem[i]->modified  &&  physmem[i]->reference) loc3 = i;       	
    }
