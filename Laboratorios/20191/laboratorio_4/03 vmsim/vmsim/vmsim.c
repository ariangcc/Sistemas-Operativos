...
void simulate() {
  ...
  while (fscanf(fin, "%d, %c, %x", &pid, &ch, &vaddr) != EOF) {
    ...
    print_Line(100,'=');
    printf("Referencia nro %d con pid:%d  type:%c  vaddr:0x%x  vfn:0x%x (primeros %d bits del %d-bit vaddr)\n",
    ...
    //si la referencia sera de tipo escritura se actualiza
    if (type == REF_KIND_STORE)
      pte->modified = TRUE;
