...
void *xmalloc (size_t nbytes)
{
	...
	*/
	...
	for (p = prevp->s.ptr; ; prevp = p, p = p->s.ptr) {
		...
		if(p == freep && worst != NULL){			
			//if(best->s.size == nunits || best->s.size - nunits < 1 + eqHeader)
            if(worst->s.size == nunits || worst->s.size - nunits < 2)
				prevWorst->s.ptr = worst->s.ptr;
			else{
				worst->s.size -= nunits; 
				worst += worst->s.size;
				worst->s.size = nunits;
...
void xfree(void *ap)
{
	...
	*/	
	...
	//para WORST FIT el freep->s.ptr sera siempre el punto de inicio de la busqueda, 
