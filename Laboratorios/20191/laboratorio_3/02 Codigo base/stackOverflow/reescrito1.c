...
int main(void)
{
    ...
    visualize("hacer todos los malloc");
...
void *mmalloc(size_t nbytes)
{
    ...
    /*  (+1 additional chunk for the Header itself)
     needed to hold nbytes */
...
void _mfree(void **ap)
{
    ...
    *ap = NULL;
    ...
    /* (bp > p && bp < p->s.next)    => between two nodes */
    /* (p > p->s.next)               => this is the end of the list */
